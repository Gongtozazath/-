...
int stringLength = numOfValsRec * digitPerValRec + 1; //$00000
int counter = 0;
...
void receieveData(){
  ...
  if (counterStart){
    ...
    if (counter >=stringLength){
      ...
      {
        int num = (i*digitasPerValsRec)+1
      valsRec[i] = receivedString.substring(num,num + digitasPerValsRec).toInt();)
      }
      receivedString = "";
      counter = 0;
      counterStart = false;
...
void loop() {
    ...
    if (valsRec[1] == 1){servoIndex.write(180);}else{servoIndexwrite(0);}
